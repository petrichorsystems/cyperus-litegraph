...
;
    ...
    LGraph.prototype.removeLink = function(link_id) {
        ...
        if (node) {
            node.disconnectInput(link.target_slot, link_id, node);
        }
    };
...
*/
		    ...
		    var input = this.inputs[i];
                    ...
                    for (var j = 0; j < input.links.length; ++j) {
                        var link_info = this.graph
                            ? this.graph.links[input.links[j]]
                            : null;
    ...
    LGraphNode.prototype.isInputConnected = function(slot) {
        ...
        return (
            ...
            this.inputs[slot].links &&
            this.inputs[slot].links.length
        );
    };

    /**
     * tells you if there is any connection in the input slots
     * @method isAnyInputConnected
     * @return {boolean}
     */
    LGraphNode.prototype.isAnyInputConnected = function() {
        if (!this.inputs) {
            return false;
        }
        for (var i = 0; i < this.inputs.length; ++i) {
    ...
    /**
     * retrieves all the nodes connected to this input slot
     * @method getInputNodes
     * @param {number} slot
     * @return {array}
     */
    LGraphNode.prototype.getInputNodes = function(slot) {
     ...
     * @param {Object} extra_info this can be used to have special properties of an input (label, special color, position, etc)
     */
    ...
    LGraphNode.prototype.removeInput = function(slot) {
        ...
        for (var i = slot; i < this.inputs.length; ++i) {
            if (!this.inputs[i] || !this.inputs[i].links) {
                continue;
            }
            var links = this.inputs[i].links;
            for (var j = 0; j < links.length; ++j) {
                var link = this.graph.links[links[j]];
                if (!link) {
                    continue;
        ...
        if (target_node) {
            ...
            for (var i = 0, l = output.links.length; i < l; i++) {
                ...
                if (target_node) {
                    ...
                    input.links = null; //remove other side link
                    if (target_node.onConnectionsChange) {
                        target_node.onConnectionsChange(
                            LiteGraph.INPUT,
                            link_info.target_slot,
    ...
    /**
     * disconnect one input to an specific node
     * @method disconnectInput
     * @param {number_or_string} slot (could be the number of the slot or the string with the name of the slot)
     * @param {LGraphNode} target_node the target node to which this slot is connected [Optional, if not target_node is specified all nodes will be disconnected]
     * @return {boolean} if it was disconnected successfully
     */
    LGraphNode.prototype.disconnectInput = function(slot, linkid, target_node) {
        if (slot.constructor === String) {
        ...
        //one of the input links in this slot
        if (target_node) {
            if (target_node.constructor === Number) {
                target_node = this.graph.getNodeById(target_node);
            }
	    ...
	    // END CYPERUS CODE
                ...
                if (link_info.target_id == target_node.id &&
		   link_info.id == linkid) {
                    input.links.splice(i, 1); //remove here

		    if( target_node.outputs ) {
			var output = target_node.outputs[link_info.target_slot];
			if( target_node.outputs.links ) {
			    var output_idx = output.links.indexOf(link_id);
			    if( output_idx > -1 ) {
				output.links.splice(output_idx, 1);
			    }
			}
		    }
		    
                    // var output = target_node.outputs[link_info.target_slot];
                    // output.link = null; //remove there
        ...
        } //all the links in this input slot
        else {
            for (var i = 0, l = input.links.length; i < l; i++) {
                var link_id = input.links[i];
                var link_info = this.graph.links[link_id];
		...
		var output = null;
			...
			if( output_idx > -1 ) {
			    output.links.splice(output_idx, 1);
                    ...
                    //output = target_node.outputs[link_info.target_slot];
                    //output.links = null; //remove other side link
...
(function(global) {
	...
	// need to add bus port logic here
	// if a bus has bus ports, add them to the drawn objects if need be
    ...
    GraphInput.desc = "input of the bus";
    ...
    GraphOutput.desc = "output of the bus";
