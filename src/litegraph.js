class Cyperus {
	...
	self._waitForConnection(function () {
		...
		console.log('literaph.js::_send(), CALLBACK UNDEFINED');
	    }
	}, 1000);
    };
        ...
        for (var i = 0; i < links.length; i++) {
            ...
            if (links[i]) {
                ...
                if (cyperus_id_out && cyperus_id_in) {
	                ...
	                cyperus_id_out,
	                cyperus_id_in,
	                _cyperus_util_store_dsp_connection_id_in_litegraph_link_from_load_configure,
	                links[i]
	            );
                }
            }
...
;
    ...
    function _cyperus_util_store_new_dsp_connection_id_in_litegraph_link(response, args) {
        ...
        cyperus_connection_id = response[5];
    ...
    function _cyperus_util_store_dsp_connection_id_in_litegraph_link_from_load_configure(response, args) {
        console.log('_cyperus_util_store_new_dsp_connection_id_in_litegraph_link_from_load_configure()');

        var cyperus_connection_id = response[5];
        var link = args;

        link.cyperus_id = cyperus_connection_id;
    }
	        ...
	        //         args);
                ...
                if (!node.type.localeCompare("network/oscsend")) {
                    ...
                    if (!from_load_configure)
                        ...
                        node['properties']['open'] = configure_payload['bus_n_info'].properties['open'];
...
*/
	...
	// START CYPERUS CODE
		...
		undefined
            ...
            this.properties['reset'] = this.widgets[0].value;
            this.properties['start'] = this.widgets[1].value;
            this.properties['step_size'] = this.widgets[2].value;
            this.properties['min'] = this.widgets[3].value;
            this.properties['max'] = this.widgets[4].value;
            this.properties['direction'] = this.widgets[5].value;
            this.properties['auto_reset'] = this.widgets[6].value;
		...
		undefined
            ...
            this.properties['x'] = this.widgets[0].value;
            this.properties['y'] = this.widgets[1].value;
		...
		undefined
            ...
            this.properties['open'] = this.widgets[0].value;
    ...
    */
	...
	if(link_id != null)
	{
		...
		if (!target_node) {
                    console.log('early return, no target node');
		    return false;
		}
		...
		for (var i = 0, l = output.links.length; i < l; i++) {
			...
			break;
                ...
                console.log(this.graph.links[link_id]);
	...
	*/
	...
	root.clear = function()
	{
        ...
        root.buildFileSystemPathList = function(response, args) {
            ...
            root.graphcanvas = args['graphcanvas'];
            root.editor = args['editor'];
            
            this.panel = args['panel'];
            this.graph = args['graph'];
            this.graphcanvas = args['graphcanvas'];
	...
	root.className = "litegraph popup-dialog popup-panel";
	root.innerHTML = "<div class='popup-dialog-header'><span class='popup-dialog-title'></span></div><div class='popup-dialog-content'></div><div class='popup-dialog-footer'></div>";
	root.header = root.querySelector(".popup-dialog-header");
        root.header.onmousedown = dragMouseDown;
	...
	root.title_element = root.querySelector(".popup-dialog-title");
	root.title_element.innerText = title;
	root.content = root.querySelector(".popup-dialog-content");
	root.footer = root.querySelector(".popup-dialog-footer");;
	...
	root.clear = function()
	{
        ...
        root.addToolsButton = function( id, name, materials_icon_name, callback, args, container_name ) {
            if (!container_name) {
                container_name = "popup-tools-left";
            }
        ...
        root.createSpan = function(name, materials_icon_name, callback, args) {
            ...
            button.classList.add("popup-btn-primary");    
            button.classList.add("material-icons");
            button.classList.add("icons");
            button.style.setProperty('--variation', `'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 48`);
            ...
            if(callback)
	        button.addEventListener('click', function() {
                    root.addPopupPanelNavButtonHandler(args);
                });
            return button;
        }        
